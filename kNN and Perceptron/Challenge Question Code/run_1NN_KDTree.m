...
error_rate_KD{i,j}{k} = OneNearestNeighbour_KDTree(x_train{i,j}, y_train{i,j}, x_test{i,j}, y_test{i,j});


end
        
averaged_error_KD(i,j) = mean (cell2mat(error_rate_KD{i,j}));
    end
end
...
a = find(averaged_error_KD(:,it) < 10);
idx(it) = a(1);
end
...
grid on
title('Estimated Sample Complexity for 1-NN Classifier with KD Trees')
xlabel('Dimension')
